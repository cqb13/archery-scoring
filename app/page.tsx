  ...
  return (
    <main className="flex items-center justify-center pt-4">
      <section className="flex flex-col gap-2 items-center">
        <Dropdown title="Location" items={["Indoor", "Outdoor"]} setSelected={updateLocation} />
        <div className="flex gap-2 items-center">
          <Slider title="Distance" jump={1} defaultValue={18} min={1} max={100} update={updateDistance} />
          <Dropdown title="Distance Unit" items={["M (meters)", "YD (yards)", "FT (feet)"]} setSelected={updateDistanceUnit} />
        </div>
        <Slider title="Ends" jump={1} defaultValue={10} min={1} max={40} update={updateEnds} />
        <Slider title="Arrow Per End" jump={1} defaultValue={3} min={1} max={12} update={updateArrowsPerEnd} />
        <Slider title="Split Ends" jump={1} defaultValue={1} min={1} max={4} update={updateSplitEnds} />
        <Dropdown title="Bow" items={["Barebow", "Olympic Recurve", "Compound"]} setSelected={updateBow} />
        <Button title="Start Scoring" onClick={startScoring}/>
      </section>
      {error ? <ErrorPopup title="Error" message={errorMessage} timeout={5000} updateError={updateError}/> : null}
    </main>
  )
}
